...
NAME
    ...
    wl-exec -- execute Grenchmark composite applications
...
def runJob(data):
    ...
    if  sleepDuration <= 0: # job is due to run
        ...
        # 1. check if the job can be run (time)
        JobID = data['id']
	print "##### [runJob] job is due to run"
        ...
        if CurrentCompositeApplicationData.isRunnable(JobID): # job can run
            # 1.1. yes: run the job
            print "##### [runJob] 1.1 yes, running the job!"
            ...
            if data['NoSubmit'] != 1:
                try:
                    ...
                    if CurrentCompositeApplicationData.isFailed(JobID):
                        ...
                        ##print "Job", JobID, "Send None result"
                        return None
...
def runWL( OutputDir, XMLhandler, NPoolThreads, NoSubmit=0, Background=0, OneFile=0 ):
    ...
    print "Starting a thread pool with", NPoolThreads, "threads"
...
##            #print "Main thread working..."
            ...
            if TheCompositeApplicationData.isCompositeApplicationFinished():
                ...
                print "WARNING! About to finish: got ASPNThreadPool.NoResultsPending"

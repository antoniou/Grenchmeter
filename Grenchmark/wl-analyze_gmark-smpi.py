...
def parseSMPIOutputFile( FileName ):
    ##print "parseSMPIOutputFile", "::parsing", FileName
#      Name | Times | Avg (s) | Min (s) | Max (s) | Total (s)  
#    Overall|      8|  18.602|  18.601|  18.603| 148.817
#      Input|      0|   0.000|   0.000|   0.000|   0.000
#     Output|      0|   0.000|   0.000|   0.000|   0.000
#     Memory|40960000|   0.000|   0.000|   0.008|  31.169
#Computation|20480000|   0.000|   0.000|   0.008|  37.605
#Communication|     80|   0.409|   0.318|   0.546|  32.741
#    Barrier|     80|   0.012|   0.000|   0.033|   0.948 
    RunTime = None
    
    ListOfTableKeyWords = ['Overall', 'Input', 'Output', 'Memory', 
                           'Computation', 'Communication', 'Barrier']
    RunTimes = {}
    for KeyWord in ListOfTableKeyWords:
        RunTimes[KeyWord] = { 'Times':None,'Avg':None,'Min':None,'Max':None,'Total':None }
    TableSeparator = '|'
    LineNo = 0
    InFileName = FileName
    InFile = open( InFileName )
    ...
    while 1:
        ...
        for line in lines: 
            ...
            if line.find('Runtime') >= 0:
                ...
                if pos2 < 0: continue
                try:
                    RunTime = float(line[pos1+1:pos1+1+pos2].strip())
                except:
                    print '>>>', line[pos1+1:pos1+1+pos2].strip()
                    pass
            elif line.find(TableSeparator) >= 0:
                try:
                    name, times, avg, min, max, total = line.split(TableSeparator, 5)
                    name = name.strip()
                    times = int(times.strip())
                    # +1 : Hack to make number of seconds correct 0.44 -> 1, not 0 seconds !!!!
                    avg = int(float(avg.strip()) + 1.0)
                    min = int(float(min.strip()) + 1.0)
                    max = int(float(max.strip()) + 1.0)
                    total = int(float(total.strip()) + 1.0)
                    if name in ListOfTableKeyWords:
                        RunTimes[name] = { 'Times':times,'Avg':avg,'Min':min,'Max':max,'Total':total }
                        ##print 'Found', name, RunTimes[name]
                    ##else:
                    ##    print 'Wrong keyword:', name
                except:
                    pass

Generator="""
    Name=exe
    Info=Workload for a stand-alone executable application
    Author=M.I.Andreica
...
class SSERComponent:
    ...
    # TODO corina change this hardcoded part
    SSER_Exe = os.path.join( "/tmp", "ssert" )
    #SSER_Exe = os.path.join( "/tmp", "simple_exe" )
...
#            ArgsList.append("--nooutput")
    ...
    def generateComponent( self, bGenerateRandom = 1, \
        ...
        ComponentDirName = "%s_exxe" % self.ComponentData['id']
        ...
        self.ComponentData['stdout'] = os.path.join( FullComponentDirName, "sser-%s.out" % self.ComponentData['id'] )
        self.ComponentData['stderr'] = os.path.join( FullComponentDirName, "sser-%s.err" % self.ComponentData['id'] )
        ...
        self.ComponentData['arguments'] = \
            self.generateArgsList( InputSize, InputSize, OutputSize, OutputSize, OutputSize, WLUnit )
...
def generateJobInfo( UnitDir, UnitID, JobIndex, WLUnit, SubmitDurationMS ):
    ...
    InfoDic['jdf'] = absPath
    try:
        InfoDic['submitCommand'] = WLUnit['otherinfo']['cmdline']
    except Exception, e:
        absPath = os.path.abspath(InfoDic['jdf'])
        InfoDic['jdf'] = absPath
        InfoDic['submitCommand'] = 'python /home/antoniou/Cmeter/src/ec2br/ec2br.py -p 3000 -j %s' % absPath    
    try:
...
def generateJobComponents( UnitDir, UnitID, JobIndex, WLUnit, bGenerateRandom = 1, \
    ...
    print "### [exe-wlgen] generateJobComponents"
    ComponentsList = []
    nValidComponents = 0
    for ComponentID in WLUnit['components'].keys():
        ...
        if OneSSER.generateComponent( bGenerateRandom, Size, WLUnit, MaxWallTime ) >= 0: 
...
def generateWorkloadUnit( UnitDir, UnitID, WLUnit, SubmitDurationMS, bGenerateRandom = 1 ):
    ...
    #TODO corina modify this hardcoded part
    #SSERComponent.SSER_Exe = os.path.join( "/tmp", "ssert" )
    SSERComponent.SSER_Exe = os.path.join( "/tmp", "simple_exe" )
    ...
    print "### [exe-wlgen] generateWorkloadUnit"

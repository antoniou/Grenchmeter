...
def parseNQueensOutputFile( FileName ):
    #took 77.402 s
    RunTime = None
    LineNo = 0
    InFileName = FileName
    ...
    while 1:
        ...
        for line in lines: 
            ...
            if line.find('took') >= 0:
                ...
                pos1 = line.find('took')
                if pos1 < 0: continue
                pos2 = line[pos1+len('took'):].find('s')
                if pos2 < 0: continue
                try:
                    RunTime = int(float(line[pos1+len('took'):pos1+len('took')+pos2].strip()) + 1.0)
                except:
                ...
                if JobResult == 'SUCCESS': 
                    ...
                    if os.path.exists(DataItem['jdf']):
                        ...
                        for FileName in OutputFilesList:
                            ...
                            RunTime = parseNQueensOutputFile( DirFileName )
                            if RunTime:
